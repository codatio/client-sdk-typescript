...
import { connectionManagementCorsSettingsGet } from "@codat/platform/funcs/connectionManagementCorsSettingsGet.js";
...
async function run() {
  const res = await connectionManagementCorsSettingsGet(codatPlatform);
...
```typescript
import { CodatPlatformCore } from "@codat/platform/core.js";
import { connectionManagementCorsSettingsSet } from "@codat/platform/funcs/connectionManagementCorsSettingsSet.js";
...
async function run() {
  const res = await connectionManagementCorsSettingsSet(codatPlatform, {
    allowedOrigins: [
      "https://www.bank-of-dave.com",
    ],
  });
