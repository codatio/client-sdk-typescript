/*
 * Code generated by Speakeasy (https://speakeasy.com). DO NOT EDIT.
 */

import { pushDataListOperations } from "../../funcs/pushDataListOperations.js";
import * as operations from "../../sdk/models/operations/index.js";
import { formatResult, ToolDefinition } from "../tools.js";

const args = {
  request: operations.GetCompanyPushHistoryRequest$inboundSchema,
};

export const tool$pushDataListOperations: ToolDefinition<typeof args> = {
  name: "push-data-list-operations",
  description: `List push operations

The **List push operations** endpoint returns a list of [push operations](/using-the-api/push) performed on the company.

Write operations are actions that send requests to Codat, enabling the creation, updating, deletion of records, or uploading attachments in the connected accounting software.`,
  args,
  tool: async (client, args, ctx) => {
    const [result, apiCall] = await pushDataListOperations(
      client,
      args.request,
      { fetchOptions: { signal: ctx.signal } },
    ).$inspect();

    if (!result.ok) {
      return {
        content: [{ type: "text", text: result.error.message }],
        isError: true,
      };
    }

    const value = result.value;

    return formatResult(value, apiCall);
  },
};
