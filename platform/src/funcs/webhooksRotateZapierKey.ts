...
type Call = CodatPlatform["webhooks"]["rotateZapierKey"];

export function webhooksRotateZapierKey(
    client: CodatPlatformCore,
    request?: Record<string, never>,
    options?: Parameters<Call>[0],
): APIPromise<Result<Awaited<ReturnType<Call>>, Error>> {
    void request;
    return new APIPromise(toResult(() => client.webhooks.rotateZapierKey(options)));
}
