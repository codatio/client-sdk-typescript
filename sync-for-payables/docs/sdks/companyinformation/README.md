...
async function run() {
  const result = await codatSyncPayables.companyInformation.get({
...
import { companyInformationGet } from "@codat/sync-for-payables/funcs/companyInformationGet.js";
...
async function run() {
  const res = await companyInformationGet(codatSyncPayables, {
    companyId: "8a210b68-6988-11ed-a1eb-0242ac120002",
    connectionId: "2e9d2c44-f675-40ba-8049-353bfcb5e171",
  });
  if (res.ok) {
    ...
    console.log("companyInformationGet failed:", res.error);
  }
}
