...
export function listBillsRequestFromJSON(
  jsonString: string,
): SafeParseResult<ListBillsRequest, SDKValidationError> {
  return safeParse(
    jsonString,
    (x) => ListBillsRequest$inboundSchema.parse(JSON.parse(x)),
    `Failed to parse 'ListBillsRequest' from JSON`,
  );
}
