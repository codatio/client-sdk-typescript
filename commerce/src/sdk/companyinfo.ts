  ...
  **/
  getCommerceInfo(
    ...
    return r.then((httpRes: AxiosResponse) => {
        ...
        const res: operations.GetCommerceInfoResponse = {statusCode: httpRes.status, contentType: contentType, rawResponse: httpRes};
        switch (true) {
          case httpRes?.status == 200:
            if (utils.matchContentType(contentType, `application/json`)) {
              res.sourceModifiedDate = plainToInstance(
                operations.GetCommerceInfoSourceModifiedDate,
                httpRes?.data as operations.GetCommerceInfoSourceModifiedDate,
