  ...
  **/
  listCommerceLocations(
    ...
    return r.then((httpRes: AxiosResponse) => {
        ...
        const res: operations.ListCommerceLocationsResponse = {statusCode: httpRes.status, contentType: contentType, rawResponse: httpRes};
        switch (true) {
          case httpRes?.status == 200:
            if (utils.matchContentType(contentType, `application/json`)) {
              res.links = plainToInstance(
                operations.ListCommerceLocationsLinks,
                httpRes?.data as operations.ListCommerceLocationsLinks,
