  ...
  **/
  listCommerceOrders(
    ...
    return r.then((httpRes: AxiosResponse) => {
        ...
        const res: operations.ListCommerceOrdersResponse = {statusCode: httpRes.status, contentType: contentType, rawResponse: httpRes};
        switch (true) {
          case httpRes?.status == 200:
            if (utils.matchContentType(contentType, `application/json`)) {
              res.links = plainToInstance(
                operations.ListCommerceOrdersLinks,
                httpRes?.data as operations.ListCommerceOrdersLinks,
