...
export class Sync {
   ...
   */
  postSyncHistoric(
    req: operations.PostSyncHistoricRequest,
    config?: AxiosRequestConfig
  ): Promise<operations.PostSyncHistoricResponse> {
    if (!(req instanceof utils.SpeakeasyBase)) {
      req = new operations.PostSyncHistoricRequest(req);
    }
    ...
    return r.then((httpRes: AxiosResponse) => {
      ...
      const res: operations.PostSyncHistoricResponse =
        new operations.PostSyncHistoricResponse({
      ...
      switch (true) {
        case httpRes?.status == 200:
          if (utils.matchContentType(contentType, `application/json`)) {
            res.postSyncHistoric200ApplicationJSONObject =
              utils.deserializeJSONResponse(
                httpRes?.data,
                operations.PostSyncHistoric200ApplicationJSON
              );
          }
          break;
      }
