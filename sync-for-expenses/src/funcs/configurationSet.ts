...
import { CodatSyncExpensesError } from "../sdk/models/errors/codatsyncexpenseserror.js";
import {
  ConnectionError,
  InvalidRequestError,
  RequestAbortedError,
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.SetCompanyConfigurationRequest$outboundSchema.parse(value),
  ...
  const body = encodeJSON("body", payload.CompanyConfiguration, {
    explode: true,
  });
  ...
  const path = pathToFunc("/companies/{companyId}/sync/expenses/config")(
    pathParams,
  );
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, shared.CompanyConfiguration$inboundSchema),
    M.jsonErr(
