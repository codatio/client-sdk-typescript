  ...
  **/
  getCompaniesCompanyIdConnectionsConnectionIdPush(
    ...
    if (!(req instanceof utils.SpeakeasyBase)) {
      req = new operations.GetCompaniesCompanyIdConnectionsConnectionIdPushRequest(req);
    }
    ...
    return r.then((httpRes: AxiosResponse) => {
        ...
        const res: operations.GetCompaniesCompanyIdConnectionsConnectionIdPushResponse = {statusCode: httpRes.status, contentType: contentType};
        switch (true) {
          case httpRes?.status == 200:
            if (utils.matchContentType(contentType, `application/json`)) {
                res.pushOption = httpRes?.data;
            }
            break;
        }
  ...
  getCompaniesCompanyIdPushPushOperationKey(
    ...
    return r.then((httpRes: AxiosResponse) => {
        ...
        switch (true) {
          case httpRes?.status == 200:
            if (utils.matchContentType(contentType, `application/json`)) {
                res.getCompaniesCompanyIdPushPushOperationKey200ApplicationJSONObject = httpRes?.data;
            }
            break;
        }
