...
type Call = CodatLending["bankStatements"]["getCategorizedBankStatementTransactions"];

export function bankStatementsGetCategorizedBankStatementTransactions(
    client: CodatLendingCore,
    request: Parameters<Call>[0],
    options?: Parameters<Call>[1],
): APIPromise<Result<Awaited<ReturnType<Call>>, Error>> {
    return new APIPromise(toResult(() => client.bankStatements.getCategorizedBankStatementTransactions(request, options)));
}
