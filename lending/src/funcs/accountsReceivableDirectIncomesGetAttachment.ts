...
type Call = CodatLending["accountsReceivable"]["directIncomes"]["getAttachment"];

export function accountsReceivableDirectIncomesGetAttachment(
    client: CodatLendingCore,
    request: Parameters<Call>[0],
    options?: Parameters<Call>[1],
): APIPromise<Result<Awaited<ReturnType<Call>>, Error>> {
    return new APIPromise(toResult(() => client.accountsReceivable.directIncomes.getAttachment(request, options)));
}
