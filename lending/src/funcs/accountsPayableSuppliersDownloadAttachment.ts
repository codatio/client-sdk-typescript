...
type Call = CodatLending["accountsPayable"]["suppliers"]["downloadAttachment"];

export function accountsPayableSuppliersDownloadAttachment(
    client: CodatLendingCore,
    request: Parameters<Call>[0],
    options?: Parameters<Call>[1],
): APIPromise<Result<Awaited<ReturnType<Call>>, Error>> {
    return new APIPromise(toResult(() => client.accountsPayable.suppliers.downloadAttachment(request, options)));
}
