...
type Call = CodatLending["banking"]["accounts"]["list"];

export function bankingAccountsList(
    client: CodatLendingCore,
    request: Parameters<Call>[0],
    options?: Parameters<Call>[1],
): APIPromise<Result<Awaited<ReturnType<Call>>, Error>> {
    return new APIPromise(toResult(() => client.banking.accounts.list(request, options)));
}
