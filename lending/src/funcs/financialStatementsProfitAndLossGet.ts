...
type Call = CodatLending["financialStatements"]["profitAndLoss"]["get"];

export function financialStatementsProfitAndLossGet(
    client: CodatLendingCore,
    request: Parameters<Call>[0],
    options?: Parameters<Call>[1],
): APIPromise<Result<Awaited<ReturnType<Call>>, Error>> {
    return new APIPromise(toResult(() => client.financialStatements.profitAndLoss.get(request, options)));
}
