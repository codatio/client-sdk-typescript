...
type Call = CodatLending["accountsPayable"]["billCreditNotes"]["list"];

export function accountsPayableBillCreditNotesList(
    client: CodatLendingCore,
    request: Parameters<Call>[0],
    options?: Parameters<Call>[1],
): APIPromise<Result<Awaited<ReturnType<Call>>, Error>> {
    return new APIPromise(toResult(() => client.accountsPayable.billCreditNotes.list(request, options)));
}
