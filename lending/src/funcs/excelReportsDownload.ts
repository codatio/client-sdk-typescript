...
type Call = CodatLending["excelReports"]["download"];

export function excelReportsDownload(
    client: CodatLendingCore,
    request: Parameters<Call>[0],
    options?: Parameters<Call>[1],
): APIPromise<Result<Awaited<ReturnType<Call>>, Error>> {
    return new APIPromise(toResult(() => client.excelReports.download(request, options)));
}
