...
type Call = CodatLending["liabilities"]["generateLoanSummary"];

export function liabilitiesGenerateLoanSummary(
    client: CodatLendingCore,
    request: Parameters<Call>[0],
    options?: Parameters<Call>[1],
): APIPromise<Result<Awaited<ReturnType<Call>>, Error>> {
    return new APIPromise(toResult(() => client.liabilities.generateLoanSummary(request, options)));
}
