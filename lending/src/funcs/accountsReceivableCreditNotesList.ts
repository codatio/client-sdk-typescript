...
type Call = CodatLending["accountsReceivable"]["creditNotes"]["list"];

export function accountsReceivableCreditNotesList(
    client: CodatLendingCore,
    request: Parameters<Call>[0],
    options?: Parameters<Call>[1],
): APIPromise<Result<Awaited<ReturnType<Call>>, Error>> {
    return new APIPromise(toResult(() => client.accountsReceivable.creditNotes.list(request, options)));
}
