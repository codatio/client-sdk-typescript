...
type Call = CodatLending["financialStatements"]["accounts"]["get"];

export function financialStatementsAccountsGet(
    client: CodatLendingCore,
    request: Parameters<Call>[0],
    options?: Parameters<Call>[1],
): APIPromise<Result<Awaited<ReturnType<Call>>, Error>> {
    return new APIPromise(toResult(() => client.financialStatements.accounts.get(request, options)));
}
