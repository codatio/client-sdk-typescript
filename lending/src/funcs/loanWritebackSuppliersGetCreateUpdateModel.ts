...
type Call = CodatLending["loanWriteback"]["suppliers"]["getCreateUpdateModel"];

export function loanWritebackSuppliersGetCreateUpdateModel(
    client: CodatLendingCore,
    request: Parameters<Call>[0],
    options?: Parameters<Call>[1],
): APIPromise<Result<Awaited<ReturnType<Call>>, Error>> {
    return new APIPromise(toResult(() => client.loanWriteback.suppliers.getCreateUpdateModel(request, options)));
}
