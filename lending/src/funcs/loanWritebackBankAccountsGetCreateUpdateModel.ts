...
type Call = CodatLending["loanWriteback"]["bankAccounts"]["getCreateUpdateModel"];

export function loanWritebackBankAccountsGetCreateUpdateModel(
    client: CodatLendingCore,
    request: Parameters<Call>[0],
    options?: Parameters<Call>[1],
): APIPromise<Result<Awaited<ReturnType<Call>>, Error>> {
    return new APIPromise(toResult(() => client.loanWriteback.bankAccounts.getCreateUpdateModel(request, options)));
}
