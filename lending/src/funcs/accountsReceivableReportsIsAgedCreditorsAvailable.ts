...
type Call = CodatLending["accountsReceivable"]["reports"]["isAgedCreditorsAvailable"];

export function accountsReceivableReportsIsAgedCreditorsAvailable(
    client: CodatLendingCore,
    request: Parameters<Call>[0],
    options?: Parameters<Call>[1],
): APIPromise<Result<Awaited<ReturnType<Call>>, Error>> {
    return new APIPromise(toResult(() => client.accountsReceivable.reports.isAgedCreditorsAvailable(request, options)));
}
