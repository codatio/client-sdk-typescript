...
type Call = CodatLending["excelReports"]["generate"];

export function excelReportsGenerate(
    client: CodatLendingCore,
    request: Parameters<Call>[0],
    options?: Parameters<Call>[1],
): APIPromise<Result<Awaited<ReturnType<Call>>, Error>> {
    return new APIPromise(toResult(() => client.excelReports.generate(request, options)));
}
