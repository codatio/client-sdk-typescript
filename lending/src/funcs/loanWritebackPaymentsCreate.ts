...
type Call = CodatLending["loanWriteback"]["payments"]["create"];

export function loanWritebackPaymentsCreate(
    client: CodatLendingCore,
    request: Parameters<Call>[0],
    options?: Parameters<Call>[1],
): APIPromise<Result<Awaited<ReturnType<Call>>, Error>> {
    return new APIPromise(toResult(() => client.loanWriteback.payments.create(request, options)));
}
