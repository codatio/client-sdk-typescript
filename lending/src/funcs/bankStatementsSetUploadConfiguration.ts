...
type Call = CodatLending["bankStatements"]["setUploadConfiguration"];

export function bankStatementsSetUploadConfiguration(
    client: CodatLendingCore,
    request: Parameters<Call>[0],
    options?: Parameters<Call>[1],
): APIPromise<Result<Awaited<ReturnType<Call>>, Error>> {
    return new APIPromise(toResult(() => client.bankStatements.setUploadConfiguration(request, options)));
}
