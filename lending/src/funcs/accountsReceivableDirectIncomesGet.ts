...
type Call = CodatLending["accountsReceivable"]["directIncomes"]["get"];

export function accountsReceivableDirectIncomesGet(
    client: CodatLendingCore,
    request: Parameters<Call>[0],
    options?: Parameters<Call>[1],
): APIPromise<Result<Awaited<ReturnType<Call>>, Error>> {
    return new APIPromise(toResult(() => client.accountsReceivable.directIncomes.get(request, options)));
}
