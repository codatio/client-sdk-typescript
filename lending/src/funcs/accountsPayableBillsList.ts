...
type Call = CodatLending["accountsPayable"]["bills"]["list"];

export function accountsPayableBillsList(
    client: CodatLendingCore,
    request: Parameters<Call>[0],
    options?: Parameters<Call>[1],
): APIPromise<Result<Awaited<ReturnType<Call>>, Error>> {
    return new APIPromise(toResult(() => client.accountsPayable.bills.list(request, options)));
}
