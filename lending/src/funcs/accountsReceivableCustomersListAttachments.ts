...
type Call = CodatLending["accountsReceivable"]["customers"]["listAttachments"];

export function accountsReceivableCustomersListAttachments(
    client: CodatLendingCore,
    request: Parameters<Call>[0],
    options?: Parameters<Call>[1],
): APIPromise<Result<Awaited<ReturnType<Call>>, Error>> {
    return new APIPromise(toResult(() => client.accountsReceivable.customers.listAttachments(request, options)));
}
