...
type Call = CodatLending["accountsPayable"]["billPayments"]["list"];

export function accountsPayableBillPaymentsList(
    client: CodatLendingCore,
    request: Parameters<Call>[0],
    options?: Parameters<Call>[1],
): APIPromise<Result<Awaited<ReturnType<Call>>, Error>> {
    return new APIPromise(toResult(() => client.accountsPayable.billPayments.list(request, options)));
}
