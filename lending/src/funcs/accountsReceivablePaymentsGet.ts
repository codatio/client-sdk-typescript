...
type Call = CodatLending["accountsReceivable"]["payments"]["get"];

export function accountsReceivablePaymentsGet(
    client: CodatLendingCore,
    request: Parameters<Call>[0],
    options?: Parameters<Call>[1],
): APIPromise<Result<Awaited<ReturnType<Call>>, Error>> {
    return new APIPromise(toResult(() => client.accountsReceivable.payments.get(request, options)));
}
