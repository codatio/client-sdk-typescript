...
type Call = CodatLending["accountsPayable"]["billCreditNotes"]["get"];

export function accountsPayableBillCreditNotesGet(
    client: CodatLendingCore,
    request: Parameters<Call>[0],
    options?: Parameters<Call>[1],
): APIPromise<Result<Awaited<ReturnType<Call>>, Error>> {
    return new APIPromise(toResult(() => client.accountsPayable.billCreditNotes.get(request, options)));
}
