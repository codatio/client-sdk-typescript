...
type Call = CodatLending["accountsReceivable"]["directIncomes"]["downloadAttachment"];

export function accountsReceivableDirectIncomesDownloadAttachment(
    client: CodatLendingCore,
    request: Parameters<Call>[0],
    options?: Parameters<Call>[1],
): APIPromise<Result<Awaited<ReturnType<Call>>, Error>> {
    return new APIPromise(toResult(() => client.accountsReceivable.directIncomes.downloadAttachment(request, options)));
}
