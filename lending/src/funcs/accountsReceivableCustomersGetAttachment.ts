...
type Call = CodatLending["accountsReceivable"]["customers"]["getAttachment"];

export function accountsReceivableCustomersGetAttachment(
    client: CodatLendingCore,
    request: Parameters<Call>[0],
    options?: Parameters<Call>[1],
): APIPromise<Result<Awaited<ReturnType<Call>>, Error>> {
    return new APIPromise(toResult(() => client.accountsReceivable.customers.getAttachment(request, options)));
}
