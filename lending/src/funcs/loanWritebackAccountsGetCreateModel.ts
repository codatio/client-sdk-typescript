...
type Call = CodatLending["loanWriteback"]["accounts"]["getCreateModel"];

export function loanWritebackAccountsGetCreateModel(
    client: CodatLendingCore,
    request: Parameters<Call>[0],
    options?: Parameters<Call>[1],
): APIPromise<Result<Awaited<ReturnType<Call>>, Error>> {
    return new APIPromise(toResult(() => client.loanWriteback.accounts.getCreateModel(request, options)));
}
