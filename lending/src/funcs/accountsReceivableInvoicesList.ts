...
type Call = CodatLending["accountsReceivable"]["invoices"]["list"];

export function accountsReceivableInvoicesList(
    client: CodatLendingCore,
    request: Parameters<Call>[0],
    options?: Parameters<Call>[1],
): APIPromise<Result<Awaited<ReturnType<Call>>, Error>> {
    return new APIPromise(toResult(() => client.accountsReceivable.invoices.list(request, options)));
}
