...
type Call = CodatLending["loanWriteback"]["bankTransactions"]["create"];

export function loanWritebackBankTransactionsCreate(
    client: CodatLendingCore,
    request: Parameters<Call>[0],
    options?: Parameters<Call>[1],
): APIPromise<Result<Awaited<ReturnType<Call>>, Error>> {
    return new APIPromise(toResult(() => client.loanWriteback.bankTransactions.create(request, options)));
}
