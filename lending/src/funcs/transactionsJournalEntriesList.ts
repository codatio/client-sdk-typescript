 ...
 */
export async function transactionsJournalEntriesList(
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.ListAccountingJournalEntriesRequest$outboundSchema.parse(
        value,
      ),
  ...
  const path = pathToFunc("/companies/{companyId}/data/journalEntries")(
    pathParams,
  );
  ...
  const [result] = await M.match<
    ...
    | ConnectionError
  >(
    M.json(200, shared.AccountingJournalEntries$inboundSchema),
    M.jsonErr(
