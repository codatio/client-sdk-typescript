...
type Call = CodatLending["manageData"]["pullOperations"]["list"];

export function manageDataPullOperationsList(
    client: CodatLendingCore,
    request: Parameters<Call>[0],
    options?: Parameters<Call>[1],
): APIPromise<Result<Awaited<ReturnType<Call>>, Error>> {
    return new APIPromise(toResult(() => client.manageData.pullOperations.list(request, options)));
}
