...
type Call = CodatLending["transactions"]["directCosts"]["list"];

export function transactionsDirectCostsList(
    client: CodatLendingCore,
    request: Parameters<Call>[0],
    options?: Parameters<Call>[1],
): APIPromise<Result<Awaited<ReturnType<Call>>, Error>> {
    return new APIPromise(toResult(() => client.transactions.directCosts.list(request, options)));
}
