...
type Call = CodatLending["bankStatements"]["uploadBankStatementData"];

export function bankStatementsUploadBankStatementData(
    client: CodatLendingCore,
    request: Parameters<Call>[0],
    options?: Parameters<Call>[1],
): APIPromise<Result<Awaited<ReturnType<Call>>, Error>> {
    return new APIPromise(toResult(() => client.bankStatements.uploadBankStatementData(request, options)));
}
