...
type Call = CodatLending["financialSummary"]["downloadCreditModelExcel"];

export function financialSummaryDownloadCreditModelExcel(
    client: CodatLendingCore,
    request: Parameters<Call>[0],
    options?: Parameters<Call>[1],
): APIPromise<Result<Awaited<ReturnType<Call>>, Error>> {
    return new APIPromise(toResult(() => client.financialSummary.downloadCreditModelExcel(request, options)));
}
