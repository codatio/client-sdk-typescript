...
type Call = CodatLending["liabilities"]["getLoanSummary"];

export function liabilitiesGetLoanSummary(
    client: CodatLendingCore,
    request: Parameters<Call>[0],
    options?: Parameters<Call>[1],
): APIPromise<Result<Awaited<ReturnType<Call>>, Error>> {
    return new APIPromise(toResult(() => client.liabilities.getLoanSummary(request, options)));
}
