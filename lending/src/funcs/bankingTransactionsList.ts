...
type Call = CodatLending["banking"]["transactions"]["list"];

export function bankingTransactionsList(
    client: CodatLendingCore,
    request: Parameters<Call>[0],
    options?: Parameters<Call>[1],
): APIPromise<Result<Awaited<ReturnType<Call>>, Error>> {
    return new APIPromise(toResult(() => client.banking.transactions.list(request, options)));
}
