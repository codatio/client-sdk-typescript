...
type Call = CodatLending["transactions"]["directCosts"]["get"];

export function transactionsDirectCostsGet(
    client: CodatLendingCore,
    request: Parameters<Call>[0],
    options?: Parameters<Call>[1],
): APIPromise<Result<Awaited<ReturnType<Call>>, Error>> {
    return new APIPromise(toResult(() => client.transactions.directCosts.get(request, options)));
}
