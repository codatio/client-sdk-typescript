...
type Call = CodatLending["companyInfo"]["getCommerceProfile"];

export function companyInfoGetCommerceProfile(
    client: CodatLendingCore,
    request: Parameters<Call>[0],
    options?: Parameters<Call>[1],
): APIPromise<Result<Awaited<ReturnType<Call>>, Error>> {
    return new APIPromise(toResult(() => client.companyInfo.getCommerceProfile(request, options)));
}
