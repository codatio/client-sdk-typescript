...
type Call = CodatLending["financialStatements"]["cashFlow"]["get"];

export function financialStatementsCashFlowGet(
    client: CodatLendingCore,
    request: Parameters<Call>[0],
    options?: Parameters<Call>[1],
): APIPromise<Result<Awaited<ReturnType<Call>>, Error>> {
    return new APIPromise(toResult(() => client.financialStatements.cashFlow.get(request, options)));
}
