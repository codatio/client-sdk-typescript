...
type Call = CodatLending["accountsReceivable"]["directIncomes"]["list"];

export function accountsReceivableDirectIncomesList(
    client: CodatLendingCore,
    request: Parameters<Call>[0],
    options?: Parameters<Call>[1],
): APIPromise<Result<Awaited<ReturnType<Call>>, Error>> {
    return new APIPromise(toResult(() => client.accountsReceivable.directIncomes.list(request, options)));
}
