...
type Call = CodatLending["accountsReceivable"]["creditNotes"]["get"];

export function accountsReceivableCreditNotesGet(
    client: CodatLendingCore,
    request: Parameters<Call>[0],
    options?: Parameters<Call>[1],
): APIPromise<Result<Awaited<ReturnType<Call>>, Error>> {
    return new APIPromise(toResult(() => client.accountsReceivable.creditNotes.get(request, options)));
}
