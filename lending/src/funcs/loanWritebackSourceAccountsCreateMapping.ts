...
type Call = CodatLending["loanWriteback"]["sourceAccounts"]["createMapping"];

export function loanWritebackSourceAccountsCreateMapping(
    client: CodatLendingCore,
    request: Parameters<Call>[0],
    options?: Parameters<Call>[1],
): APIPromise<Result<Awaited<ReturnType<Call>>, Error>> {
    return new APIPromise(toResult(() => client.loanWriteback.sourceAccounts.createMapping(request, options)));
}
