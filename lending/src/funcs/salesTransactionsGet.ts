...
type Call = CodatLending["sales"]["transactions"]["get"];

export function salesTransactionsGet(
    client: CodatLendingCore,
    request: Parameters<Call>[0],
    options?: Parameters<Call>[1],
): APIPromise<Result<Awaited<ReturnType<Call>>, Error>> {
    return new APIPromise(toResult(() => client.sales.transactions.get(request, options)));
}
