...
type Call = CodatLending["loanWriteback"]["createOperations"]["get"];

export function loanWritebackCreateOperationsGet(
    client: CodatLendingCore,
    request: Parameters<Call>[0],
    options?: Parameters<Call>[1],
): APIPromise<Result<Awaited<ReturnType<Call>>, Error>> {
    return new APIPromise(toResult(() => client.loanWriteback.createOperations.get(request, options)));
}
