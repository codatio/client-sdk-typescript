...
type Call = CodatLending["accountsReceivable"]["customers"]["list"];

export function accountsReceivableCustomersList(
    client: CodatLendingCore,
    request: Parameters<Call>[0],
    options?: Parameters<Call>[1],
): APIPromise<Result<Awaited<ReturnType<Call>>, Error>> {
    return new APIPromise(toResult(() => client.accountsReceivable.customers.list(request, options)));
}
