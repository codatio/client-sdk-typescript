...
type Call = CodatLending["accountsReceivable"]["payments"]["list"];

export function accountsReceivablePaymentsList(
    client: CodatLendingCore,
    request: Parameters<Call>[0],
    options?: Parameters<Call>[1],
): APIPromise<Result<Awaited<ReturnType<Call>>, Error>> {
    return new APIPromise(toResult(() => client.accountsReceivable.payments.list(request, options)));
}
