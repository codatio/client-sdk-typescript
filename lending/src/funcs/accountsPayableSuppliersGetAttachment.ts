...
type Call = CodatLending["accountsPayable"]["suppliers"]["getAttachment"];

export function accountsPayableSuppliersGetAttachment(
    client: CodatLendingCore,
    request: Parameters<Call>[0],
    options?: Parameters<Call>[1],
): APIPromise<Result<Awaited<ReturnType<Call>>, Error>> {
    return new APIPromise(toResult(() => client.accountsPayable.suppliers.getAttachment(request, options)));
}
