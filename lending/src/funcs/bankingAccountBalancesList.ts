...
type Call = CodatLending["banking"]["accountBalances"]["list"];

export function bankingAccountBalancesList(
    client: CodatLendingCore,
    request: Parameters<Call>[0],
    options?: Parameters<Call>[1],
): APIPromise<Result<Awaited<ReturnType<Call>>, Error>> {
    return new APIPromise(toResult(() => client.banking.accountBalances.list(request, options)));
}
