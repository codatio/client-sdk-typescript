...
type Call = CodatLending["sales"]["reports"]["getRefunds"];

export function salesReportsGetRefunds(
    client: CodatLendingCore,
    request: Parameters<Call>[0],
    options?: Parameters<Call>[1],
): APIPromise<Result<Awaited<ReturnType<Call>>, Error>> {
    return new APIPromise(toResult(() => client.sales.reports.getRefunds(request, options)));
}
