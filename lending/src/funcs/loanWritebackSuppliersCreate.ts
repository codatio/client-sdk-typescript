...
type Call = CodatLending["loanWriteback"]["suppliers"]["create"];

export function loanWritebackSuppliersCreate(
    client: CodatLendingCore,
    request: Parameters<Call>[0],
    options?: Parameters<Call>[1],
): APIPromise<Result<Awaited<ReturnType<Call>>, Error>> {
    return new APIPromise(toResult(() => client.loanWriteback.suppliers.create(request, options)));
}
