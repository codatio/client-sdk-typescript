...
type Call = CodatLending["excelReports"]["getStatus"];

export function excelReportsGetStatus(
    client: CodatLendingCore,
    request: Parameters<Call>[0],
    options?: Parameters<Call>[1],
): APIPromise<Result<Awaited<ReturnType<Call>>, Error>> {
    return new APIPromise(toResult(() => client.excelReports.getStatus(request, options)));
}
