...
type Call = CodatLending["accountsPayable"]["suppliers"]["list"];

export function accountsPayableSuppliersList(
    client: CodatLendingCore,
    request: Parameters<Call>[0],
    options?: Parameters<Call>[1],
): APIPromise<Result<Awaited<ReturnType<Call>>, Error>> {
    return new APIPromise(toResult(() => client.accountsPayable.suppliers.list(request, options)));
}
