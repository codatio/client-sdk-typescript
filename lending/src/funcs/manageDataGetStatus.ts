...
type Call = CodatLending["manageData"]["getStatus"];

export function manageDataGetStatus(
    client: CodatLendingCore,
    request: Parameters<Call>[0],
    options?: Parameters<Call>[1],
): APIPromise<Result<Awaited<ReturnType<Call>>, Error>> {
    return new APIPromise(toResult(() => client.manageData.getStatus(request, options)));
}
