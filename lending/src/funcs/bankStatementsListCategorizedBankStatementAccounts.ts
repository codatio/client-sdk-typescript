...
type Call = CodatLending["bankStatements"]["listCategorizedBankStatementAccounts"];

export function bankStatementsListCategorizedBankStatementAccounts(
    client: CodatLendingCore,
    request: Parameters<Call>[0],
    options?: Parameters<Call>[1],
): APIPromise<Result<Awaited<ReturnType<Call>>, Error>> {
    return new APIPromise(toResult(() => client.bankStatements.listCategorizedBankStatementAccounts(request, options)));
}
