...
type Call = CodatLending["loanWriteback"]["bankTransactions"]["getCreateModel"];

export function loanWritebackBankTransactionsGetCreateModel(
    client: CodatLendingCore,
    request: Parameters<Call>[0],
    options?: Parameters<Call>[1],
): APIPromise<Result<Awaited<ReturnType<Call>>, Error>> {
    return new APIPromise(toResult(() => client.loanWriteback.bankTransactions.getCreateModel(request, options)));
}
