...
type Call = CodatLending["banking"]["transactionCategories"]["list"];

export function bankingTransactionCategoriesList(
    client: CodatLendingCore,
    request: Parameters<Call>[0],
    options?: Parameters<Call>[1],
): APIPromise<Result<Awaited<ReturnType<Call>>, Error>> {
    return new APIPromise(toResult(() => client.banking.transactionCategories.list(request, options)));
}
