...
type Call = CodatLending["banking"]["transactionCategories"]["get"];

export function bankingTransactionCategoriesGet(
    client: CodatLendingCore,
    request: Parameters<Call>[0],
    options?: Parameters<Call>[1],
): APIPromise<Result<Awaited<ReturnType<Call>>, Error>> {
    return new APIPromise(toResult(() => client.banking.transactionCategories.get(request, options)));
}
