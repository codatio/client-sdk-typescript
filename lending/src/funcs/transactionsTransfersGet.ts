...
type Call = CodatLending["transactions"]["transfers"]["get"];

export function transactionsTransfersGet(
    client: CodatLendingCore,
    request: Parameters<Call>[0],
    options?: Parameters<Call>[1],
): APIPromise<Result<Awaited<ReturnType<Call>>, Error>> {
    return new APIPromise(toResult(() => client.transactions.transfers.get(request, options)));
}
