...
type Call = CodatLending["bankStatements"]["downloadCategorizedBankStatementExcel"];

export function bankStatementsDownloadCategorizedBankStatementExcel(
    client: CodatLendingCore,
    request: Parameters<Call>[0],
    options?: Parameters<Call>[1],
): APIPromise<Result<Awaited<ReturnType<Call>>, Error>> {
    return new APIPromise(toResult(() => client.bankStatements.downloadCategorizedBankStatementExcel(request, options)));
}
