...
type Call = CodatLending["transactions"]["directCosts"]["getAttachment"];

export function transactionsDirectCostsGetAttachment(
    client: CodatLendingCore,
    request: Parameters<Call>[0],
    options?: Parameters<Call>[1],
): APIPromise<Result<Awaited<ReturnType<Call>>, Error>> {
    return new APIPromise(toResult(() => client.transactions.directCosts.getAttachment(request, options)));
}
