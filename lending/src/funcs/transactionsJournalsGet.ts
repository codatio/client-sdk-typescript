...
type Call = CodatLending["transactions"]["journals"]["get"];

export function transactionsJournalsGet(
    client: CodatLendingCore,
    request: Parameters<Call>[0],
    options?: Parameters<Call>[1],
): APIPromise<Result<Awaited<ReturnType<Call>>, Error>> {
    return new APIPromise(toResult(() => client.transactions.journals.get(request, options)));
}
