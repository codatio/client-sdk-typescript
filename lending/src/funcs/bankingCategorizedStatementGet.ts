...
type Call = CodatLending["banking"]["categorizedStatement"]["get"];

export function bankingCategorizedStatementGet(
    client: CodatLendingCore,
    request: Parameters<Call>[0],
    options?: Parameters<Call>[1],
): APIPromise<Result<Awaited<ReturnType<Call>>, Error>> {
    return new APIPromise(toResult(() => client.banking.categorizedStatement.get(request, options)));
}
