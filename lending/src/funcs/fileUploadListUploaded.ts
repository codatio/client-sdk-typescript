...
type Call = CodatLending["fileUpload"]["listUploaded"];

export function fileUploadListUploaded(
    client: CodatLendingCore,
    request: Parameters<Call>[0],
    options?: Parameters<Call>[1],
): APIPromise<Result<Awaited<ReturnType<Call>>, Error>> {
    return new APIPromise(toResult(() => client.fileUpload.listUploaded(request, options)));
}
