...
type Call = CodatLending["loanWriteback"]["directCosts"]["create"];

export function loanWritebackDirectCostsCreate(
    client: CodatLendingCore,
    request: Parameters<Call>[0],
    options?: Parameters<Call>[1],
): APIPromise<Result<Awaited<ReturnType<Call>>, Error>> {
    return new APIPromise(toResult(() => client.loanWriteback.directCosts.create(request, options)));
}
