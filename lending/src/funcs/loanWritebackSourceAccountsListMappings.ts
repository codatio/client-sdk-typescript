...
type Call = CodatLending["loanWriteback"]["sourceAccounts"]["listMappings"];

export function loanWritebackSourceAccountsListMappings(
    client: CodatLendingCore,
    request: Parameters<Call>[0],
    options?: Parameters<Call>[1],
): APIPromise<Result<Awaited<ReturnType<Call>>, Error>> {
    return new APIPromise(toResult(() => client.loanWriteback.sourceAccounts.listMappings(request, options)));
}
