 ...
 */
export async function loanWritebackSourceAccountsListMappings(
  client: CodatLendingCore,
  request: operations.GetBankAccountMappingRequest,
  options?: RequestOptions,
): Promise<
  Result<
    Array<shared.BankFeedMapping>,
    | errors.ErrorMessage
    | SDKError
    | SDKValidationError
    | UnexpectedClientError
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.GetBankAccountMappingRequest$outboundSchema.parse(value),
  ...
  const [result] = await M.match<
    ...
    | ConnectionError
  >(
    M.json(200, z.array(shared.BankFeedMapping$inboundSchema)),
    M.jsonErr(
