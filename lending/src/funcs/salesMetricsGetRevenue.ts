...
type Call = CodatLending["sales"]["metrics"]["getRevenue"];

export function salesMetricsGetRevenue(
    client: CodatLendingCore,
    request: Parameters<Call>[0],
    options?: Parameters<Call>[1],
): APIPromise<Result<Awaited<ReturnType<Call>>, Error>> {
    return new APIPromise(toResult(() => client.sales.metrics.getRevenue(request, options)));
}
