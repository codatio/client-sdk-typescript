...
type Call = CodatLending["accountsPayable"]["bills"]["listAttachments"];

export function accountsPayableBillsListAttachments(
    client: CodatLendingCore,
    request: Parameters<Call>[0],
    options?: Parameters<Call>[1],
): APIPromise<Result<Awaited<ReturnType<Call>>, Error>> {
    return new APIPromise(toResult(() => client.accountsPayable.bills.listAttachments(request, options)));
}
