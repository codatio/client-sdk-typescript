...
type Call = CodatLending["transactions"]["accountTransactions"]["get"];

export function transactionsAccountTransactionsGet(
    client: CodatLendingCore,
    request: Parameters<Call>[0],
    options?: Parameters<Call>[1],
): APIPromise<Result<Awaited<ReturnType<Call>>, Error>> {
    return new APIPromise(toResult(() => client.transactions.accountTransactions.get(request, options)));
}
