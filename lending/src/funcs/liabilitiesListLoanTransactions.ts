...
type Call = CodatLending["liabilities"]["listLoanTransactions"];

export function liabilitiesListLoanTransactions(
    client: CodatLendingCore,
    request: Parameters<Call>[0],
    options?: Parameters<Call>[1],
): APIPromise<Result<Awaited<ReturnType<Call>>, Error>> {
    return new APIPromise(toResult(() => client.liabilities.listLoanTransactions(request, options)));
}
