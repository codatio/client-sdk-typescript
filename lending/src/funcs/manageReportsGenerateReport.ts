...
type Call = CodatLending["manageReports"]["generateReport"];

export function manageReportsGenerateReport(
    client: CodatLendingCore,
    request: Parameters<Call>[0],
    options?: Parameters<Call>[1],
): APIPromise<Result<Awaited<ReturnType<Call>>, Error>> {
    return new APIPromise(toResult(() => client.manageReports.generateReport(request, options)));
}
