...
type Call = CodatLending["loanWriteback"]["directCosts"]["getCreateModel"];

export function loanWritebackDirectCostsGetCreateModel(
    client: CodatLendingCore,
    request: Parameters<Call>[0],
    options?: Parameters<Call>[1],
): APIPromise<Result<Awaited<ReturnType<Call>>, Error>> {
    return new APIPromise(toResult(() => client.loanWriteback.directCosts.getCreateModel(request, options)));
}
