...
type Call = CodatLending["financialStatements"]["profitAndLoss"]["getCategorizedAccounts"];

export function financialStatementsProfitAndLossGetCategorizedAccounts(
    client: CodatLendingCore,
    request: Parameters<Call>[0],
    options?: Parameters<Call>[1],
): APIPromise<Result<Awaited<ReturnType<Call>>, Error>> {
    return new APIPromise(toResult(() => client.financialStatements.profitAndLoss.getCategorizedAccounts(request, options)));
}
