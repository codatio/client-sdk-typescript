

## 2023-09-01 16:53:11
### Changes
Based on:
- OpenAPI Doc 3.0.0 https://raw.githubusercontent.com/codatio/oas/main/yaml/Codat-Lending.yaml
- Speakeasy CLI 1.77.1 (2.91.4) https://github.com/speakeasy-api/speakeasy
### Generated
- [typescript v0.1.0] lending
### Releases
- [NPM v0.1.0] https://www.npmjs.com/package/@codat/lending/v/0.1.0 - lending

## 2023-09-13 12:15:13
### Changes
Based on:
- OpenAPI Doc 3.0.0 https://raw.githubusercontent.com/codatio/oas/main/yaml/Codat-Lending.yaml
- Speakeasy CLI 1.83.0 (2.109.1) https://github.com/speakeasy-api/speakeasy
### Generated
- [typescript v1.1.0] lending
### Releases
- [NPM v1.1.0] https://www.npmjs.com/package/@codat/lending/v/1.1.0 - lending

## 2023-09-14 14:06:45
### Changes
Based on:
- OpenAPI Doc 3.0.0 https://raw.githubusercontent.com/codatio/oas/main/yaml/Codat-Lending.yaml
- Speakeasy CLI 1.83.2 (2.109.3) https://github.com/speakeasy-api/speakeasy
### Generated
- [typescript v1.2.0] lending
### Releases
- [NPM v1.2.0] https://www.npmjs.com/package/@codat/lending/v/1.2.0 - lending

## 2023-09-14 15:08:43
### Changes
Based on:
- OpenAPI Doc 3.0.0 https://raw.githubusercontent.com/codatio/oas/main/yaml/Codat-Lending.yaml
- Speakeasy CLI 1.83.2 (2.109.3) https://github.com/speakeasy-api/speakeasy
### Generated
- [typescript v2.1.0] lending
### Releases
- [NPM v2.1.0] https://www.npmjs.com/package/@codat/lending/v/2.1.0 - lending

## 2023-09-18 22:03:36
### Changes
Based on:
- OpenAPI Doc 3.0.0 https://raw.githubusercontent.com/codatio/oas/main/yaml/Codat-Lending.yaml
- Speakeasy CLI 1.87.0 (2.116.0) https://github.com/speakeasy-api/speakeasy
### Generated
- [typescript v3.1.0] lending
### Releases
- [NPM v3.1.0] https://www.npmjs.com/package/@codat/lending/v/3.1.0 - lending

## 2023-09-20 21:31:39
### Changes
Based on:
- OpenAPI Doc 3.0.0 https://raw.githubusercontent.com/codatio/oas/main/yaml/Codat-Lending.yaml
- Speakeasy CLI 1.88.1 (2.122.1) https://github.com/speakeasy-api/speakeasy
### Generated
- [typescript v3.2.0] lending
### Releases
- [NPM v3.2.0] https://www.npmjs.com/package/@codat/lending/v/3.2.0 - lending

## 2023-10-17 14:45:37
### Changes
Based on:
- OpenAPI Doc 3.0.0 https://raw.githubusercontent.com/codatio/oas/main/yaml/Codat-Lending.yaml
- Speakeasy CLI 1.100.2 (2.159.2) https://github.com/speakeasy-api/speakeasy
### Generated
- [typescript v3.3.0] lending
### Releases
- [NPM v3.3.0] https://www.npmjs.com/package/@codat/lending/v/3.3.0 - lending

## 2023-12-05 11:14:49
### Changes
Based on:
- OpenAPI Doc 3.0.0 https://raw.githubusercontent.com/codatio/oas/main/yaml/Codat-Lending.yaml
- Speakeasy CLI 1.125.1 (2.210.3) https://github.com/speakeasy-api/speakeasy
### Generated
- [typescript v4.0.0] lending
### Releases
- [NPM v4.0.0] https://www.npmjs.com/package/@codat/lending/v/4.0.0 - lending

## 2023-12-14 14:06:18
### Changes
Based on:
- OpenAPI Doc 3.0.0 https://raw.githubusercontent.com/codatio/oas/main/yaml/Codat-Lending.yaml
- Speakeasy CLI 1.126.3 (2.214.3) https://github.com/speakeasy-api/speakeasy
### Generated
- [typescript v4.1.0] lending
### Releases
- [NPM v4.1.0] https://www.npmjs.com/package/@codat/lending/v/4.1.0 - lending

## 2023-12-15 12:54:56
### Changes
Based on:
- OpenAPI Doc 3.0.0 https://raw.githubusercontent.com/codatio/oas/main/yaml/Codat-Lending.yaml
- Speakeasy CLI 1.126.3 (2.214.3) https://github.com/speakeasy-api/speakeasy
### Generated
- [typescript v4.2.0] lending
### Releases
- [NPM v4.2.0] https://www.npmjs.com/package/@codat/lending/v/4.2.0 - lending

## 2024-02-14 10:09:39
### Changes
Based on:
- OpenAPI Doc 3.0.0 https://raw.githubusercontent.com/codatio/oas/main/yaml/Codat-Lending.yaml
- Speakeasy CLI 1.178.1 (2.253.0) https://github.com/speakeasy-api/speakeasy
### Generated
- [typescript v4.3.0] lending
### Releases
- [NPM v4.3.0] https://www.npmjs.com/package/@codat/lending/v/4.3.0 - lending

## 2024-11-06 15:03:15
### Changes
Based on:
- OpenAPI Doc  
- Speakeasy CLI 1.431.0 (2.451.0) https://github.com/speakeasy-api/speakeasy
### Generated
- [typescript v5.0.0] lending
### Releases
- [NPM v5.0.0] https://www.npmjs.com/package/@codat/lending/v/5.0.0 - lending

## 2024-11-18 11:29:43
### Changes
Based on:
- OpenAPI Doc  
- Speakeasy CLI 1.440.1 (2.460.1) https://github.com/speakeasy-api/speakeasy
### Generated
- [typescript v6.0.0] lending
### Releases
- [NPM v6.0.0] https://www.npmjs.com/package/@codat/lending/v/6.0.0 - lending

## 2024-11-26 17:02:22
### Changes
Based on:
- OpenAPI Doc  
- Speakeasy CLI 1.447.0 (2.463.0) https://github.com/speakeasy-api/speakeasy
### Generated
- [typescript v7.0.0] lending
### Releases
- [NPM v7.0.0] https://www.npmjs.com/package/@codat/lending/v/7.0.0 - lending

## 2025-01-06 09:10:53
### Changes
Based on:
- OpenAPI Doc  
- Speakeasy CLI 1.462.2 (2.486.1) https://github.com/speakeasy-api/speakeasy
### Generated
- [typescript v7.1.0] lending
### Releases
- [NPM v7.1.0] https://www.npmjs.com/package/@codat/lending/v/7.1.0 - lending

## 2025-01-10 17:34:50
### Changes
Based on:
- OpenAPI Doc  
- Speakeasy CLI 1.468.0 (2.492.3) https://github.com/speakeasy-api/speakeasy
### Generated
- [typescript v7.2.0] lending
### Releases
- [NPM v7.2.0] https://www.npmjs.com/package/@codat/lending/v/7.2.0 - lending

## 2025-03-19 11:43:55
### Changes
Based on:
- OpenAPI Doc  
- Speakeasy CLI 1.520.1 (2.556.1) https://github.com/speakeasy-api/speakeasy
### Generated
- [typescript v7.3.0] lending
### Releases
- [NPM v7.3.0] https://www.npmjs.com/package/@codat/lending/v/7.3.0 - lending

## 2026-01-21 14:07:28
### Changes
Based on:
- OpenAPI Doc  
- Speakeasy CLI 1.692.0 (2.797.1) https://github.com/speakeasy-api/speakeasy
### Generated
- [typescript v7.4.0] lending
### Releases
- [NPM v7.4.0] https://www.npmjs.com/package/@codat/lending/v/7.4.0 - lending

## 2026-02-04 11:25:51
### Changes
Based on:
- OpenAPI Doc  
- Speakeasy CLI 1.701.0 (2.803.3) https://github.com/speakeasy-api/speakeasy
### Generated
- [typescript v7.5.0] lending
### Releases
- [NPM v7.5.0] https://www.npmjs.com/package/@codat/lending/v/7.5.0 - lending

## 2026-04-22 11:07:25
### Changes
Based on:
- OpenAPI Doc  
- Speakeasy CLI 1.761.9 (2.881.4) https://github.com/speakeasy-api/speakeasy
### Generated
- [typescript v7.6.0] lending
### Releases
- [NPM v7.6.0] https://www.npmjs.com/package/@codat/lending/v/7.6.0 - lending

## 2026-08-25 08:41:51
### Changes
Based on:
- OpenAPI Doc 3.0.0 https://raw.githubusercontent.com/codatio/oas/main/yaml/Codat-Lending.yaml
- codat sdk-codegen 3a25ff845e20563b384e461ca6e593d0aaefac77 https://github.com/codat-spikes/sdk-codegen
### Generated
- [typescript v7.6.1] lending
### Releases
- [NPM v7.6.1] https://www.npmjs.com/package/@codat/lending/v/7.6.1 - lending

### Notes
- First release of `@codat/lending` generated by Codat's own SDK pipeline rather than Speakeasy. The client class, method names, parameters and model type names match v7.6.0, so ordinary use needs no code change.
- Breaking: a path into the package no longer resolves. v7.6.0 put its compiled JavaScript at the package root mirroring `src/` and declared no `exports` field, so `@codat/lending/sdk/models/shared/company.js` worked. This release compiles to `dist/` and names the paths it exports: the package root, `sdk/models/shared`, `sdk/models/operations`, `sdk/models/webhooks`, `sdk/models/errors`, `sdk/types`, `core.js`, `lib/http`, `funcs/*.js` and `package.json`.
- Model types come from the package root now, so `import { Company } from "@codat/lending"` replaces the old path. v7.6.0 exported no models from the root, which is why reaching inside the package was the only way to get at them.
- The `zod` validation schemas that sat beside those types are gone, and `zod` is no longer a dependency. In place of v7.6.0's `Company$inboundSchema` and `companyFromJSON`, each model exports `CompanyFromJSON`, `CompanyToJSON` and `instanceOfCompany`.
- `SDKError.message` now holds only the message itself. v7.6.0 appended the status code, the content type and the response body to it. All three are still on the error, as `statusCode`, `contentType` and `body`.
- Requests and parsed responses were compared against v7.6.0 across 295 operation runs, and they match. One difference remains: a `oneOf` response body that matches none of its declared shapes comes back as an empty object, where v7.6.0 threw. On every request this build sends a `user-agent` naming Codat's generator instead of Speakeasy's, no empty `cookie` header, and query parameters can come out in a different order.
