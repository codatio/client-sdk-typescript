...
/** @internal */
export const SyncRange$inboundSchema: z.ZodType<
  SyncRange,
  z.ZodTypeDef,
  unknown
> = z.object({
...
/** @internal */
export const SyncRange$outboundSchema: z.ZodType<
  SyncRange$Outbound,
  z.ZodTypeDef,
  SyncRange
> = z.object({
...
export function syncRangeToJSON(syncRange: SyncRange): string {
  return JSON.stringify(SyncRange$outboundSchema.parse(syncRange));
}

export function syncRangeFromJSON(
  jsonString: string,
): SafeParseResult<SyncRange, SDKValidationError> {
  return safeParse(
    jsonString,
    (x) => SyncRange$inboundSchema.parse(JSON.parse(x)),
    `Failed to parse 'SyncRange' from JSON`,
  );
}
