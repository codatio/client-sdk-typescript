...
export class BillPayments {
  ...
  **/
  getBillPayments(
    ...
    return r.then((httpRes: AxiosResponse) => {
        ...
        const res: operations.GetBillPaymentsResponse = {statusCode: httpRes.status, contentType: contentType};
        switch (true) {
  ...
  **/
  listBillPayments(
    ...
    return r.then((httpRes: AxiosResponse) => {
        ...
        if (httpRes?.status == null) throw new Error(`status code not found in response: ${httpRes}`);
        const res: operations.ListBillPaymentsResponse = {statusCode: httpRes.status, contentType: contentType};
        switch (true) {
          case httpRes?.status == 200:
            if (utils.matchContentType(contentType, `application/json`)) {
  ...
  **/
  postBillPayment(
    req: operations.PostBillPaymentRequest,
    config?: AxiosRequestConfig
  ): Promise<operations.PostBillPaymentResponse> {
    if (!(req instanceof utils.SpeakeasyBase)) {
      req = new operations.PostBillPaymentRequest(req);
    }
    ...
    return r.then((httpRes: AxiosResponse) => {
        ...
        if (httpRes?.status == null) throw new Error(`status code not found in response: ${httpRes}`);
        const res: operations.PostBillPaymentResponse = {statusCode: httpRes.status, contentType: contentType};
        switch (true) {
          case httpRes?.status == 200:
            if (utils.matchContentType(contentType, `application/json`)) {
              res.postBillPayment200ApplicationJSONObject = plainToInstance(
                operations.PostBillPayment200ApplicationJSON,
                httpRes?.data as operations.PostBillPayment200ApplicationJSON,
                { excludeExtraneousValues: true }
              );
            }
            break;
