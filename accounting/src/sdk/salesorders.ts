  ...
  **/
  getSalesOrder(
    ...
    return r.then((httpRes: AxiosResponse) => {
        ...
        const res: operations.GetSalesOrderResponse = {statusCode: httpRes.status, contentType: contentType, rawResponse: httpRes};
        switch (true) {
          case httpRes?.status == 200:
            if (utils.matchContentType(contentType, `application/json`)) {
              res.sourceModifiedDate = plainToInstance(
                operations.GetSalesOrderSourceModifiedDate,
                httpRes?.data as operations.GetSalesOrderSourceModifiedDate,
  ...
  **/
  listSalesOrders(
    ...
    return r.then((httpRes: AxiosResponse) => {
        ...
        if (httpRes?.status == null) throw new Error(`status code not found in response: ${httpRes}`);
        const res: operations.ListSalesOrdersResponse = {statusCode: httpRes.status, contentType: contentType, rawResponse: httpRes};
        switch (true) {
          case httpRes?.status == 200:
            if (utils.matchContentType(contentType, `application/json`)) {
              res.links = plainToInstance(
                operations.ListSalesOrdersLinks,
                httpRes?.data as operations.ListSalesOrdersLinks,
                { excludeExtraneousValues: true }
              );
            }
            break;
