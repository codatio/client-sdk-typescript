  ...
  **/
  getCompanyInfo(
    ...
    return r.then((httpRes: AxiosResponse) => {
        ...
        const res: operations.GetCompanyInfoResponse = {statusCode: httpRes.status, contentType: contentType};
        switch (true) {
          case httpRes?.status == 200:
            if (utils.matchContentType(contentType, `application/json`)) {
                res.companyInfo = httpRes?.data;
            }
            break;
        }
  ...
  **/
  postSyncInfo(
    ...
    return r.then((httpRes: AxiosResponse) => {
        ...
        if (httpRes?.status == null) throw new Error(`status code not found in response: ${httpRes}`);
        const res: operations.PostSyncInfoResponse = {statusCode: httpRes.status, contentType: contentType};
        switch (true) {
          case httpRes?.status == 200:
            if (utils.matchContentType(contentType, `application/json`)) {
                res.postSyncInfo200ApplicationJSONObject = httpRes?.data;
            }
            break;
        }
