...
> {
  ...
  const parsed$ = schemas$.safeParse(
    input$,
    (value$) =>
      operations.ListPaymentMethodsRequest$outboundSchema.parse(value$),
  ...
  const path$ = pathToFunc("/companies/{companyId}/data/paymentMethods")(
    pathParams$,
  );
  ...
  const [result$] = await m$.match<
    ...
    | ConnectionError
  >(
    m$.json(200, shared.PaymentMethods$inboundSchema),
    m$.jsonErr(
