  ...
  **/
  createExpenseDataset(
    ...
    return r.then((httpRes: AxiosResponse) => {
        ...
        const res: operations.CreateExpenseDatasetResponse = {statusCode: httpRes.status, contentType: contentType};
        switch (true) {
          case httpRes?.status == 200:
            if (utils.matchContentType(contentType, `application/json`)) {
                res.createExpenseDataset200ApplicationJSONObject = httpRes?.data;
            }
            break;
        }
  ...
  **/
  uploadAttachment(
    ...
    return r.then((httpRes: AxiosResponse) => {
        ...
        if (httpRes?.status == null) throw new Error(`status code not found in response: ${httpRes}`);
        const res: operations.UploadAttachmentResponse = {statusCode: httpRes.status, contentType: contentType};
        switch (true) {
          case httpRes?.status == 200:
            if (utils.matchContentType(contentType, `application/json`)) {
                res.uploadAttachment200ApplicationJSONObject = httpRes?.data;
            }
            break;
        }
